...
class MedianFilter
{
  ...
  static int multiMap (int val, const int *_in, const int *_out, const byte& size)
  {
  ...
  static float multiMap (float val, const float *_in, const float *_out, const byte& size)
  {
    // Take care the value is within range
    // val = constrain(val, _in[0], _in[size-1]);
    if (val <= _in[0]) return _out[0];
